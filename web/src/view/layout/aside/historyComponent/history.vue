<template>
    <div class="router-history">
      ...
      >
        <el-tab-pane
          ...
          class="gva-tab"
        >
          <span slot="label" :style="{color: activeValue===name(item)?activeTextColor:'#333'}"><i class="dot" :style="{backgroundColor:activeValue===name(item)?activeTextColor:'#ddd'}" /> {{ item.meta.title }}</span>
        </el-tab-pane>
      </el-tabs>
  ...
  export default {
    ...
    computed: {
      ...mapGetters('user', ['userPageSet', 'activeTextColor']),
      defaultRouter() {
        return this.userPageSet.defaultRouter
      }
    },
    watch: {
      contextMenuVisible() {
