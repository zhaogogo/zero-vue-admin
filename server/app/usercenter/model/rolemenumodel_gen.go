...
var (
	...
	cacheUsercenterRoleMenuIdPrefix = "cache:usercenter:roleMenu:id:"
)
...
func (m *defaultRoleMenuModel) Delete(ctx context.Context, id uint64) error {
	usercenterRoleMenuIdKey := fmt.Sprintf("%s%v", cacheUsercenterRoleMenuIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, usercenterRoleMenuIdKey)
	return err
}
...
func (m *defaultRoleMenuModel) FindOne(ctx context.Context, id uint64) (*RoleMenu, error) {
	usercenterRoleMenuIdKey := fmt.Sprintf("%s%v", cacheUsercenterRoleMenuIdPrefix, id)
	var resp RoleMenu
	err := m.QueryRowCtx(ctx, &resp, usercenterRoleMenuIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultRoleMenuModel) Insert(ctx context.Context, data *RoleMenu) (sql.Result, error) {
	usercenterRoleMenuIdKey := fmt.Sprintf("%s%v", cacheUsercenterRoleMenuIdPrefix, data.Id)
...
func (m *defaultRoleMenuModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheUsercenterRoleMenuIdPrefix, primary)
}
