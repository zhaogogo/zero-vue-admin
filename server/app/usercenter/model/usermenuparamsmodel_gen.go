...
var (
	...
	cacheUsercenterUserMenuParamsIdPrefix = "cache:usercenter:userMenuParams:id:"
)
...
func (m *defaultUserMenuParamsModel) Delete(ctx context.Context, id uint64) error {
	usercenterUserMenuParamsIdKey := fmt.Sprintf("%s%v", cacheUsercenterUserMenuParamsIdPrefix, id)
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, usercenterUserMenuParamsIdKey)
	return err
}
...
func (m *defaultUserMenuParamsModel) FindOne(ctx context.Context, id uint64) (*UserMenuParams, error) {
	...
	err := m.QueryRowCtx(ctx, &resp, usercenterUserMenuParamsIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultUserMenuParamsModel) Insert(ctx context.Context, data *UserMenuParams) (sql.Result, error) {
	usercenterUserMenuParamsIdKey := fmt.Sprintf("%s%v", cacheUsercenterUserMenuParamsIdPrefix, data.Id)
...
func (m *defaultUserMenuParamsModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheUsercenterUserMenuParamsIdPrefix, primary)
}
