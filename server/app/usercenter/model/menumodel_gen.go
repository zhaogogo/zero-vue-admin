...
var (
	...
	cacheUsercenterMenuIdPrefix       = "cache:usercenter:menu:id:"
	cacheUsercenterMenuNamePathPrefix = "cache:usercenter:menu:name:path:"
)
...
func (m *defaultMenuModel) Delete(ctx context.Context, id uint64) error {
	...
	usercenterMenuNamePathKey := fmt.Sprintf("%s%v:%v", cacheUsercenterMenuNamePathPrefix, data.Name, data.Path)
	_, err = m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		query := fmt.Sprintf("delete from %s where `id` = ?", m.table)
		return conn.ExecCtx(ctx, query, id)
	}, usercenterMenuIdKey, usercenterMenuNamePathKey)
	return err
}
...
func (m *defaultMenuModel) FindOne(ctx context.Context, id uint64) (*Menu, error) {
	usercenterMenuIdKey := fmt.Sprintf("%s%v", cacheUsercenterMenuIdPrefix, id)
	var resp Menu
	err := m.QueryRowCtx(ctx, &resp, usercenterMenuIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultMenuModel) FindOneByNamePath(ctx context.Context, name string, path string) (*Menu, error) {
	usercenterMenuNamePathKey := fmt.Sprintf("%s%v:%v", cacheUsercenterMenuNamePathPrefix, name, path)
	var resp Menu
	err := m.QueryRowIndexCtx(ctx, &resp, usercenterMenuNamePathKey, m.formatPrimary, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) (i interface{}, e error) {
...
func (m *defaultMenuModel) Insert(ctx context.Context, data *Menu) (sql.Result, error) {
	usercenterMenuIdKey := fmt.Sprintf("%s%v", cacheUsercenterMenuIdPrefix, data.Id)
	usercenterMenuNamePathKey := fmt.Sprintf("%s%v:%v", cacheUsercenterMenuNamePathPrefix, data.Name, data.Path)
...
func (m *defaultMenuModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheUsercenterMenuIdPrefix, primary)
}
