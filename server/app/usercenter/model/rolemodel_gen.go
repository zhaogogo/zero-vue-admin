...
var (
	...
	cacheUsercenterRoleIdPrefix = "cache:usercenter:role:id:"
)
...
func (m *defaultRoleModel) Delete(ctx context.Context, id int64) error {
	usercenterRoleIdKey := fmt.Sprintf("%s%v", cacheUsercenterRoleIdPrefix, id)
...
func (m *defaultRoleModel) FindOne(ctx context.Context, id int64) (*Role, error) {
	usercenterRoleIdKey := fmt.Sprintf("%s%v", cacheUsercenterRoleIdPrefix, id)
	var resp Role
	err := m.QueryRowCtx(ctx, &resp, usercenterRoleIdKey, func(ctx context.Context, conn sqlx.SqlConn, v interface{}) error {
...
func (m *defaultRoleModel) Insert(ctx context.Context, data *Role) (sql.Result, error) {
	usercenterRoleIdKey := fmt.Sprintf("%s%v", cacheUsercenterRoleIdPrefix, data.Id)
...
func (m *defaultRoleModel) formatPrimary(primary interface{}) string {
	return fmt.Sprintf("%s%v", cacheUsercenterRoleIdPrefix, primary)
}
